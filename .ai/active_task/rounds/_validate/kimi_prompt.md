...
## Round 011 Goal
Run a narrow same-tier fix for Round 010 workflow artifact issues.
...
- Size: small.
- Subtasks: 2-3 focused items.
- File limit: 4 changed files.
- Tier: T2.

## Round 011 Required Scope
- Fix `tools/ai-kimi-run.ps1` so safe validation/dry-run checks do not pollute the real round sequence by default.
- Include `tests.txt` in `state.md` Latest artifacts when the runner captures it.
- Add a short trusted-input note/comment for workflow test command execution, or otherwise constrain it if a small safe patch is obvious.
- Handle the accidental `.ai/active_task/rounds/round_011` prompt artifact safely. Do not delete user data; if cleanup is ambiguous, report and leave it.
...
3. After script changes: run a safe PowerShell validation that does not create another real round.
...
- .ai/active_task/rounds/_validate/kimi_log.md

Write a structured JSON report to:
- .ai/active_task/rounds/_validate/kimi_report.json

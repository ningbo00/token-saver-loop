...
- If commit delegation is explicitly allowed later, Kimi must stage only Codex-specified files, make a local commit only, and report `git show --stat --oneline HEAD` for Codex review.
...
Kimi should update `.ai/active_task/progress.md` at the end of every round.

# Product Brief — Gpt2Whatever

## 1. 项目名理解
**Gpt2Whatever** 在这里被重新定位为 **"Get Project To Whatever workflow"**：把一个可复用的 Kimi-Codex 半自动协作工作流，安装到任意代码项目中。
...
| **配置持久化** | default_project_config 返回内存 dict；未来是否写入 `gpt2whatever.toml` 或 `.ai/config.json`？ |

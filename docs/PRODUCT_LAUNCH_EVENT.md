# Token Saver Loop 产品发布会脚本

这份文档用于录制视频、直播发布、B 站/知乎/掘金配套文章，目标是让重度 AI 编程用户在 10-20 分钟内明白：
...
```text
Token Saver Loop 发布：别再让最贵的 AI 模型反复读文件、跑命令、试错和写进度
```
...
| 时间 | 环节 | 目标 |
|---|---|---|
| 0:00-1:30 | 开场痛点 | 让观众立刻觉得“这说的是我” |
| 1:30-4:00 | 产品概念 | 解释 reviewer / worker 分工 |
| 4:00-6:00 | 工作流动图 | 讲清楚两个模型如何来回切换 |
| 6:00-10:00 | 安装演示 | 复制 portable kit，无需安装器 |
| 10:00-15:00 | 第一轮使用演示 | reviewer 出任务，worker 执行，reviewer 验收 |
| 15:00-17:00 | 功能亮点 | evidence、round_status、project memory、red flags |
| 17:00-18:30 | 适合/不适合 | 防止误解和过度承诺 |
| 18:30-20:00 | 结尾 CTA | GitHub star、试用、反馈 |
...
```text
大家好，我今天发布一个小工具，叫 Token Saver Loop。
...
演示建议用 T0 inspect-only 任务，安全、不会改代码，适合发布会。
...
```text
我做了一个 AI 编程 Token 省钱工作流：贵模型只做 review，便宜模型负责执行
别再让 Claude/Codex 反复读文件了：Token Saver Loop 发布
重度 AI 编程用户如何减少上下文浪费：Token Saver Loop 演示
```

英文：

```text
Show HN: Token Saver Loop - stop spending premium coding models on execution noise
I built a reviewer/worker loop to reduce AI coding context waste
Token Saver Loop: keep premium coding models focused on judgment
```

...
1\. 低コスト汎用大モデル（実行側：Kimi／通义千问など）
2\. 高次推論大モデル（審査側：GPT／Claudeなど）
...
1. **ステップ1（ローカル準備）**：リポジトリ内の `portable/kimi-codex-kit` フォルダを、自分のプロジェクトのルートディレクトリにコピー＆ペーストする。

2. **ステップ2（審査モデルがタスクを配布）**：高次推論大モデルを開き、直接コピーして送信：`Read kimi-codex-kit/START_HERE.md and create a safe first worker task.`

3. **ステップ3（実行モデルが作業）**：低コスト大モデルを開き、直接コピーして送信：`Read kimi-codex-kit/KIMI_NEXT_TASK.md and execute it against this project.`

4. **ステップ4（審査モデルが検収）**：高次推論大モデルに戻り、直接コピーして送信：`The worker is done. Review the latest round evidence.`

### オプション：PowerShellショートカットでタスクを生成（定型フレーズを手入力する必要なし）
...
| `KIMI_NEXT_TASK.md` | 現在のラウンドで実行モデルに配布する具体的なタスク |
| `CODEX_CONTINUE.md` | 新規審査セッション時のコンテキストガイドファイル |
...
4. **インストール時の上書き防止**：CLIインストールには人間の確認が必要であり、ファイル競合検出を内蔵し、既存の業務コードを保護する。
...
### 10.2 Python CLIインストール（一括運用シナリオ）
...
適用シナリオ：複数リポジトリでのキット一括初期化。個人の日常利用では、ポータブルフォルダを優先する。

---

## 十一、初心者のよくあるFAQ

- **Q：Kimi＋特定のモデルを使う必要がありますか？** A：全く必要ありません。キットはあくまでデフォルトの例です。任意の「低価格実行モデル＋高価格審査モデル」の組み合わせに置き換え可能で、キット内部ファイルを変更する必要はありません。

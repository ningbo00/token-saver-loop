...
2. **ステップ2（審査モデルがタスクを配布）**：高次推論モデルを開き、直接コピーして送信：`Read token-saver-kit/START_HERE.md and create a safe first worker task.`

3. **ステップ3（実行モデルが作業）**：まず `powershell -ExecutionPolicy Bypass -File token-saver-kit/tools/tsl-run.ps1` を実行して、実際の `round_NNN` プロンプトを作成する。手動で使う場合は、低コストモデルを開き、直接コピーして送信：`Read token-saver-kit/WORKER_NEXT_TASK.md and execute it against this project.`
...
# 互換性のある任意の worker CLI で実行する。deepseek/glm/qwen/kimi などに置き換え可能
powershell -ExecutionPolicy Bypass -File token-saver-kit/tools/tsl-run.ps1 -WorkerCommand deepseek
# 実際のラウンドを作らず、プロンプトだけをプレビューする
powershell -ExecutionPolicy Bypass -File token-saver-kit/tools/tsl-run.ps1 -NoRun
```

`-NoRun` は `_validate` プレビュープロンプトのみを書き込みます。実際の worker ラウンドでは `-NoRun` を外してください。

任意のプロジェクトルートでヘルスチェック：
...
Python CLI は任意の補助ツールです。プロジェクトにファイルをインストールせず、診断、メトリクス、設定プレビューに使います。
